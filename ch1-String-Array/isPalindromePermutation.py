#!/usr/bin/env python3

import unittest
from collections import Counter


def is_permutation_of_a_palindrome_algorithm(phrase):
    table = [0 for _ in range(ord("z") - ord("a") + 1)]
    countodd = 0
    for c in phrase:
        x = char_number(c)
        if x != -1:
            table[x] += 1
            if table[x] % 2:
                countodd += 1
            else:
                countodd -= 1
    return countodd <= 1

def char_number(c):
    a = ord("a")
    z = ord("z")
    upper_a = ord("A")
    upper_z = ord("Z")
    val = ord(c)

    if a <= val <= z:
        return val - a
    if upper_a <= val <= upper_z:
        return val - upper_a

    return -1


def is_permutation_of_a_palindrome_pythonic(phrase):
    counter = Counter(phrase.replace(" ", "").lower())
    return sum([val%2 for val in counter.values()]) <= 1


class TestPalindromePermutation(unittest.TestCase):
    test_cases = [
        ("Tact Coa", True),
        ("Lays", False),
        ("aabb", True),
        ('bat bat', True),
        ('The boring Company', False),
    ]

    test_functions = [
        is_permutation_of_a_palindrome_algorithm,
        is_permutation_of_a_palindrome_pythonic,
    ]

    def test_palindrome_permutation(self):
        for testcase, expected in self.test_cases:
            for test_function in self.test_functions:
                self.assertEqual(test_function(testcase), expected)


if __name__ == '__main__':
    unittest.main()
